...
public void goInit(GameObject child_in, GameObject skel_root_in) {
	...
	lr_bg.SetVertexCount(2);
	lr_bg.SetWidth(.3f, .13f);
	lr_bg.sortingOrder = 990;
	lr_bg.SetColors(parent_script.m_BoneBGColor, parent_script.m_BoneBGColor);
	...
	lr_fg.SetVertexCount(2);
	lr_fg.SetWidth(.2f, .03f);
	lr_fg.sortingOrder = 991;
	lr_fg.SetColors(parent_script.m_BoneFGColor, parent_script.m_BoneFGColor);
